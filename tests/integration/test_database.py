...
class TestPostgreSQLOperations:
    ...
    def test_update_recording(self, db_manager, sample_recording_data):
        ...
        # Update recording
        updated = repo.update(created.id, file_name="updated_name.mp3", sha256_hash="newhash123")
    ...
    def test_create_metadata(self, db_manager, sample_recording_data, sample_metadata_items):
        ...
        for item in sample_metadata_items:
            metadata = metadata_repo.create(recording_id=recording.id, key=item["key"], value=item["value"])
    ...
    def test_tracklist_uniqueness(self, db_manager, sample_recording_data, sample_tracks):
        ...
        tracklist1 = tracklist_repo.create(recording_id=recording.id, source="source1", tracks=sample_tracks)
        assert tracklist1 is not None

        # Attempt to create second tracklist should fail
        with pytest.raises(IntegrityError):
            tracklist_repo.create(recording_id=recording.id, source="source2", tracks=sample_tracks)

    def test_cascade_delete(self, db_manager, sample_recording_data, sample_metadata_items, sample_tracks):
    ...
    def test_bulk_operations(self, db_manager):
        ...
        # Prepare bulk data
        recordings_data = [{"file_path": f"/music/track{i}.mp3", "file_name": f"track{i}.mp3"} for i in range(5)]
...
class TestNeo4jOperations:
    ...
    def test_create_recording_node(self, neo4j_repo):
        ...
        result = neo4j_repo.create_recording_node(
            recording_id=recording_id, file_name="test.mp3", file_path="/music/test.mp3"
        )
    ...
    def test_add_metadata_relationships(self, neo4j_repo, sample_metadata_items):
        ...
        for item in sample_metadata_items:
            neo4j_repo.add_metadata_relationship(recording_id, item["key"], item["value"])
    ...
    def test_add_tracklist_with_tracks(self, neo4j_repo, sample_tracks):
        ...
        neo4j_repo.add_tracklist_with_tracks(recording_id=recording_id, source="manual", tracks=sample_tracks)
