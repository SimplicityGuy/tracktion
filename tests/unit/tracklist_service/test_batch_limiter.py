...
class TestBatchRateLimiter:
    ...
    def test_calculate_optimal_rate_progressive(self):
        ...
        limiter = BatchRateLimiter(strategy=RateLimitStrategy.PROGRESSIVE, min_rate=1.0)
    ...
    def test_apply_backpressure(self):
        ...
        limiter.domain_metrics["domain1"] = DomainMetrics(domain="domain1", current_rate=20.0)
        limiter.domain_metrics["domain2"] = DomainMetrics(domain="domain2", current_rate=15.0)
    ...
    async def test_wait_for_slot_backed_off(self):
        ...
        # Set domain as backed off
        limiter.domain_metrics["example.com"] = DomainMetrics(
            domain="example.com", backoff_until=datetime.now(UTC) + timedelta(seconds=1)
        )
