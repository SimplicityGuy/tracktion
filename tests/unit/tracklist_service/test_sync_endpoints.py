...
class TestSyncControlEndpoints:
    ...
    async def test_trigger_sync_success(
        self, test_client, mock_sync_service, mock_db_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_trigger_sync_invalid_source(
    ...
    async def test_schedule_sync(
        ...
        with patch("services.tracklist_service.src.api.sync_endpoints.get_db", return_value=mock_db_session):
            response = test_client.post(
                f"/api/v1/tracklists/{tracklist_id}/sync/schedule?frequency=daily&source=all"
            )
    ...
    async def test_cancel_scheduled_sync(
        ...
        with patch("services.tracklist_service.src.api.sync_endpoints.get_db", return_value=mock_db_session):
            response = test_client.delete(
                f"/api/v1/tracklists/{tracklist_id}/sync/schedule"
            )
...
class TestVersionHistoryEndpoints:
    ...
    async def test_get_version_history(
        self, test_client, mock_version_service, mock_db_session
    ):
        ...
        with patch("services.tracklist_service.src.api.sync_endpoints.get_db", return_value=mock_db_session):
            response = test_client.get(
                f"/api/v1/tracklists/{tracklist_id}/versions?limit=20&offset=0"
            )
    ...
    async def test_get_version_details(
        ...
        with patch("services.tracklist_service.src.api.sync_endpoints.get_db", return_value=mock_db_session):
            response = test_client.get(
                f"/api/v1/tracklists/{tracklist_id}/versions/{version_id}"
            )
    ...
    async def test_compare_versions(
        ...
        with patch("services.tracklist_service.src.api.sync_endpoints.get_db", return_value=mock_db_session):
            response = test_client.get(
                f"/api/v1/tracklists/{tracklist_id}/versions/compare"
                f"?version1={version1_id}&version2={version2_id}"
            )
...
class TestConflictResolutionEndpoints:
    ...
    async def test_get_pending_conflicts(
        ...
        mock_event.conflict_data = {
            "conflicts": [{"id": "conflict1", "type": "track_modified"}]
        }
        ...
        with patch("services.tracklist_service.src.api.sync_endpoints.get_db", return_value=mock_db_session):
            response = test_client.get(
                f"/api/v1/tracklists/{tracklist_id}/conflicts"
            )
    ...
    async def test_resolve_conflicts(
        self, test_client, mock_conflict_service, mock_db_session
    ):
...
class TestAuditTrailEndpoint:
    ...
    async def test_get_audit_trail(
        self, test_client, mock_audit_service, mock_db_session
    ):
        ...
        with patch("services.tracklist_service.src.api.sync_endpoints.get_db", return_value=mock_db_session):
            response = test_client.get(
                f"/api/v1/tracklists/{tracklist_id}/audit?limit=50&offset=0"
            )
