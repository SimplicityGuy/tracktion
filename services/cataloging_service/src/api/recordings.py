...
async def get_recording_metadata(
    ...
    return await metadata_repo.get_by_recording_id(recording_id)
...
async def get_recording_tracklists(
    ...
    return await tracklist_repo.get_by_recording_id(recording_id)
...
async def search_recordings(
    ...
    if search.field == "file_name":
        return await repo.search_by_file_name(search.query, limit=search.limit)
    if search.field == "file_path":
        # Search by file path pattern
        recording = await repo.get_by_file_path(search.query)
        return [recording] if recording else []
