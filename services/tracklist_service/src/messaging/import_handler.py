...
class ImportMessageHandler:
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            raise MessageQueueError(
                f"Failed to connect to message queue: {str(e)}",
    ...
    async def publish_import_result(self, result_message: ImportResultMessage) -> bool:
        ...
        try:
            ...
            await self.exchange.publish(
                message,
                routing_key="tracklist.import.result"
            )
    ...
    async def get_queue_stats(self) -> Dict[str, Dict[str, int]]:
        ...
        for queue_name in [self.import_queue, self.import_result_queue, 
                          self.import_retry_queue, self.import_dlq]:
            try:
