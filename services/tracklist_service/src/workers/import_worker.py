...
from ..messaging.import_handler import (
    ImportJobMessage, ImportResultMessage, 
    import_message_handler, setup_import_message_handler
)
...
from ..exceptions import (
    ImportError, MatchingError, TimingError, CueGenerationError, 
    DatabaseError, MessageQueueError
)
    ...
    async def process_import_job(self, job_message: ImportJobMessage) -> None:
        ...
        try:
            ...
            adjusted_tracklist = self.timing_service.adjust_track_timings(
                tracklist=imported_tracklist,
                audio_duration_seconds=matching_result.metadata.get("duration_seconds") if matching_result.metadata else None
            )
            ...
            raise MessageQueueError(
                f"Failed to process import job: {str(e)}",
                correlation_id=correlation_id
            )
